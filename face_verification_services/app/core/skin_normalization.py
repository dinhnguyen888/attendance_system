"""
Skin tone normalization for improved face recognition accuracy
"""
import cv2
import numpy as np
...
def normalize_skin_tone(image: np.ndarray, face_coords: tuple, face_obj=None) -> np.ndarray:
    ...
    try:
        ...
        # Use facial landmarks from InsightFace to improve mask if available
        if face_obj is not None and hasattr(face_obj, 'kps') and face_obj.kps is not None:
            ...
            # Create mask from landmarks (triangle between eyes and nose)
            if len(landmarks) >= 3:
                # Get points between eyes and nose to create main skin region
        ...
        # Check if skin region exists
        if np.sum(skin_mask) == 0:
            print("Không tìm thấy vùng da, sử dụng ảnh gốc")
            return image
        
        # Define optimal default skin color for AI (in BGR)
        OPTIMAL_SKIN_COLOR_BGR = np.array([180, 160, 140], dtype=np.float32)  # Neutral skin color
        TARGET_BRIGHTNESS = 160  # Target brightness in Lab color space
        ...
        # Create soft mask for skin region
        soft_mask = cv2.GaussianBlur(skin_mask.astype(np.float32), (15, 15), 0) / 255.0
        
        # Step 1: Balance brightness (Histogram Equalization for skin region)
        ...
        if len(skin_l_values) > 0:
            ...
            # Adaptive histogram equalization for skin region with stronger parameters
            clahe = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(6,6))  # Increase clipLimit and decrease tileGridSize
            
            # Apply CLAHE only to skin region
            ...
            blend_strength = 0.9  # Increase from default
            l_channel = l_channel * (1 - soft_mask * blend_strength) + l_equalized * (soft_mask * blend_strength)
            
            # Add brightness smoothing with bilateral filter
            ...
            brightness_adjustment = (TARGET_BRIGHTNESS - new_mean_brightness) * 1.2  # Increase adjustment intensity
            ...
            # Calculate local mean and adjust each region
            kernel_size = 15
            local_mean = cv2.GaussianBlur(l_channel, (kernel_size, kernel_size), 0)
            local_diff = l_channel - local_mean
            enhanced_l = local_mean + local_diff * 0.7  # Reduce local variation
        ...
        # Calculate current average skin color in Lab
        ...
        if len(skin_pixels_lab) > 0:
            ...
            a_adjustment = (target_a - mean_a) * 1.5  # Increase color adjustment intensity
