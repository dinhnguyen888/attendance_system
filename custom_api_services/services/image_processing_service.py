"""
Image processing service for face recognition API
"""
import cv2
import numpy as np
...
def preprocess_image(image: np.ndarray, target_size: tuple = DETECTION_SIZE) -> Tuple[np.ndarray, np.ndarray]:
...
def scale_image_to_standard(image: np.ndarray, target_width: int = TARGET_IMAGE_WIDTH) -> np.ndarray:
